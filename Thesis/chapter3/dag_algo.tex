\begin{algorithm}[H]
\caption{QA DAG Agent}
\label{alg:qa-dag}
\KwIn{Question $q$}
\KwOut{Final answer $a$ and documents $\Sigma(\cdot)$}
...
\While{$t < T \ \text{and } U \neq \emptyset$}{
    ...
    \ForEach{$u \in U$}{
        ...
        \uIf{$\text{original\_ans}(u) = \emptyset$}{
            $\text{original\_ans}(u) \gets \text{ans}(u)$\;
        }
        
        \uIf{$\text{ans}(u) = \emptyset$}{
