...
\noindent To better understand their computational efficiency, we analyze the total token consumption, separating input and output tokens. Figure \ref{fig:qa_token_prompt_tech} and \ref{fig:qa_avg_token_prompt_tech} present the total and average token usage across the RAG baseline with ColBERTv2 ($k=100$), the QA Agent, the Auto DAG Agent, and the Cognitive QA Agent, evaluated on the four QA datasets. This comparison shows that more advanced language agent architectures, particularly those with cognitive features, consume substantially more tokens than simpler baselines.

\noindent Overall, the simpler QA Agent achieves the best balance between performance and cost. It consumes considerably fewer tokens than one of the strongest RAG baselines, ColBERTv2 ($k = 100$), while still delivering results that remain competitive with more advanced language agent architectures such as the Auto DAG Agent. The increased token usage of the Auto DAG Agent arises from the expansion of its working memory, which accumulates contextual information as it advances through the reasoning plan. Additionally, its procedural memory requires extra space to encode the rules that govern internal actions and updates to its episodic memory, including few-shot examples.
...
\noindent These findings highlight a key challenge for the broader adoption of language agent architectures in QA systems. The substantial token overhead not only raises inference costs but also introduces additional latency, which can be problematic in real-time or large-scale applications Understanding and mitigating these trade-offs is therefore essential for designing agents that are both effective and scalable.
...
\noindent These findings suggest that the proposed language agent architectures struggle to generalize across different question types and domains, highlighting the need for architectures capable of handling a broader range of questions to support real-world scenarios.

\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{images/locomo_error_comparison.eps}
