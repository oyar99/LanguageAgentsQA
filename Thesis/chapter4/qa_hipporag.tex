\begin{table}[ht]
    \centering
        \setlength{\tabcolsep}{3pt}
        \resizebox{\textwidth}{!}{
        \begin{tabular}{lcccccccccccccccccccc}
            \toprule & \multicolumn{4}{c}{\textbf{HotpotQA}} & \multicolumn{4}{c}{\textbf{2Wiki}} & \multicolumn{4}{c}{\textbf{MuSiQue}} & \multicolumn{4}{c}{\textbf{Average}} \\
            \cmidrule(lr){2-5} \cmidrule(lr){6-9} \cmidrule(lr){10-13} \cmidrule(lr){14-17}
             & EM & $R_1$ & $R_2$ & $L_1$ & EM & $R_1$ & $R_2$ & $L_1$ & EM & $R_1$ & $R_2$ & $L_1$ & EM & $R_1$ & $R_2$ & $L_1$ \\
            \midrule
            HippoRAG (Qwen2.5-14B) & 34.3 & 48.0 & 25.7 & 53.0 & 14.7 & 23.0 & 12.5 & 23.0 & 12.5 & 20.2 & 11.9 & 19.0 & 20.5 & 30.4 & 16.7 & 31.7 \\
            HippoRAG (GPT-4o-mini) & 46.0 & 61.0 & 34.4 & 65.0 & 35.1 & 42.0 & 27.4 & 42.0 & 25.3 & 36.7 & 22.8 & 34.0 & 35.5 & 46.6 & 28.2 & 47.0 \\
            \bottomrule
        \end{tabular}
        }
    \caption{QA performance of HippoRAG with both QWen2.5-14B-Instruct and GPT-4o-mini using Contriever as the embedding model, with $k = 5$, measured using Exact Match (EM), ROUGE-1 ($R_1$), ROUGE-2 ($R_2$), and an LLM-based score ($L_1$).}
    \label{tab:qa_hippo_rag_results}
\end{table}