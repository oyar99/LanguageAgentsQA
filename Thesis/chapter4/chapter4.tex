...
\noindent Since the designed agents do not generally execute a single search to obtain relevant passages, we only compute average \textbf{recall (R)}, \textbf{precision (P)}, and \textbf{$F_1$} for all queries considering all documents the agent used to derive the final answer. This information is shown in Table \ref{tab:retrieval_results_agents}.
...
\noindent For the agents architectures, QA results are presented in Table \ref{tab:qa_results_agents}.
