...
IRCoT is a method designed to solve multi-step questions. Given a question $q$, the system first retrieves a set of documents and then alternates between retrieval and reasoning until an answer is produced. At each step, the model generates a CoT reasoning statement, which is then used for the retrieval step. This iterative process enables the system to gather information that may not be retrieved from the original question alone \cite{trivedi-etal-2023-interleaving}.

\noindent Although simple, IRCoT shows improved performance with GPT-3 on benchmarks such as MuSiQue. IRCoT can be viewed as a simplified version of a ReAct agent, since it interleaves reasoning and retrieval. However, it does not incorporate additional language-agent components such as episodic memory, or tool use.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.95\textwidth]{images/ircot.png}\\[-0.25cm]
    \caption{IRCoT framework: (i) The system retrieves an initial set of documents used as context for an LLM, (ii) The system interleaves reasoning via CoT and retrieval to iteratively retrieve more documents until an answer is reached \cite{trivedi-etal-2023-interleaving}.}
    \label{fig:ircot}
\end{figure}
