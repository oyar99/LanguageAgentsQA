...
class InteractiveSearchEngine:
    ...
    def _search_bm25(self, query: str, top_k: int) -> List[Dict[str, Any]]:
        ...
        if not self.bm25_content_index or not self.bm25_title_index:
            raise RuntimeError(
                "BM25 indices not initialized. Call setup() first.")
        ...
        content_scores = self.bm25_content_index.get_scores(
            tokenized_query_content)
        ...
        top_k_indices = sorted(enumerate(combined_scores),
    ...
    def _search_colbert(self, query: str, top_k: int) -> List[Dict[str, Any]]:
        ...
        if not self.searcher:
            raise RuntimeError(
                "ColBERT searcher not initialized. Call setup() first.")
        ...
        # Prepare results
        results = []
        for i, (doc_id, _, score) in enumerate(zip(doc_ids, rankings, scores)):
            result = {
                'rank': i + 1,
    ...
    def print_results(self, query: str, results: List[Dict[str, Any]]):
        ...
        for result in results:
            ...
            if self.search_method == 'bm25' and 'content_score' in result and 'title_score' in result:
                print(
                    f"  Content Score: {result['content_score']:.4f} | Title Score: {result['title_score']:.4f}")
...
def main():
    ...
    print(
        f"Setting up search engine for {args.dataset} dataset using {search_method_display}...")
